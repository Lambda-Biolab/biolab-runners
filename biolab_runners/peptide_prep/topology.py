...
3. **CYS → CYX**: when a disulfide is requested, rename the
   ...
   only apply CYX to CYS residues bonded to another SG).
...
def build_modeller(config: PeptidePrepConfig) -> PreparationArtifacts:
...
def _build_minimization_system(
    ...
    energy_before_kjmol = minimization.read_potential_energy(
        ...
        system,
        positions,
        platform_name=config.openmm_platform,
    )
...
def _verify_cyclic_topology_chemistry(
    ...
    * The head N atom has exactly one bonded H (the peptide NH,
      not the N-terminal NH3+ triple).
    * The tail C atom has only one carbonyl O plus its CA and the
    ...
    # Count bonded H atoms on head N (peptide NH count).
    head_n_h_count = sum(
        1
        for atom in topology.atoms()
    ...
    if head_n_h_count != 1:
        raise RuntimeError(
            f"cyclic-topology audit: head N (residue {head_idx + 1}) has "
            f"{head_n_h_count} bonded H atoms; expected exactly 1 (the peptide "
            f"NH, not the N-terminal NH3+ triple). The end residues did not "
            f"template-match as internal peptide residues."
        )
