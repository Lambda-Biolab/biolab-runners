...
_GROMPP_AUDIT_MDP_CONTENT = (
    "; Minimal grompp -pp audit MDP for an unsolvated, finite, nonperiodic\n"
    "; peptide system. No PME, no PBC, zero cutoffs (vacuum). Per GROMACS\n"
    "; 2026.3 docs: ``pbc = no`` + all cut-offs zero + ``nstlist = 0`` is the\n"
    "; accepted nonperiodic no-cutoff configuration; ``ns-type`` is invalid\n"
    "; with ``pbc = no`` and would be rejected by the preprocessor.\n"
    "integrator       = steep\n"
    "nsteps           = 0\n"
    "nstlist          = 0\n"
