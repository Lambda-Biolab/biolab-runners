...
2. **Disulfide S-S bond addition**: PDBFixer leaves the CYX SG
   atoms as bonded partners; the topology needs an explicit
   ``topology.addBond`` between them and ``ForceField.createSystem``
...
def apply_disulfide_bonds(
    ...
    *,
    ...
    ``addMissingAtoms`` will add the bond when the two SG atoms are
