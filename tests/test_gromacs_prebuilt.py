...
def _prebuilt_meta_inline(cfg: GromacsProtocolConfig) -> dict[str, str]:
    """Inline copy of the runner's ``_prebuilt_meta`` helper for the test."""
    from biolab_runners.provenance import compute_file_digest

    return {
        "prebuilt_topology": cfg.prebuilt_topology,
        "prebuilt_coordinates": cfg.prebuilt_coordinates,
        "sha256_topology": compute_file_digest(Path(cfg.prebuilt_topology)) or "",
        "sha256_coordinates": compute_file_digest(Path(cfg.prebuilt_coordinates)) or "",
    }
