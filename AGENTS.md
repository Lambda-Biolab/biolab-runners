...
- **Run completion:** The runner decides whether a prior run is terminal from the manifest's absolute step + explicit early-abort metadata, NOT from file existence or size. A mid-production checkpoint can produce a 50 MB trajectory and tens of thousands of energy rows while the run is still in progress — file presence does not imply completion. A run is terminal when EITHER `manifest_step >= total_equil_steps + total_steps` (normal completion) OR `early_abort.json` exists with `aborted=True` AND a positive integer `abort_step` (intentional early termination). Tests that pre-populate a "complete" output directory must therefore cross the absolute-step threshold (or write a valid `early_abort.json` with a positive `abort_step`), not merely create files that look complete-shaped.
- **Early-abort marker is generation-scoped:** `early_abort.json` is the terminal marker for a run that ended via the offline-mdtraj gate. The marker's `abort_step` and `abort_ns` are the SAME absolute values the atomic save just committed to the manifest (v9 binding — the previous code recorded the invocation-local `steps_done`, an inconsistency that broke the marker contract). The marker is moved by `force=True` quarantine together with the manifest, the energy log, and the state files — a stale marker from a previous run would otherwise be re-read by `is_run_complete` and mis-classify a subsequent fresh run's intermediate checkpoint as terminal. The marker is also validated by `is_run_complete`: a missing, zero, or malformed `abort_step` is treated as "marker invalid → in progress" rather than terminal, so a corrupt marker cannot silently flip a fresh run to terminal.
