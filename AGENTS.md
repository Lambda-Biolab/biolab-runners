...
- **Atomic checkpoint:** Every `state.xml` save is paired with a `checkpoint.json` manifest written via the same `_atomic_save_checkpoint` call (both files written to `.tmp.<pid>.<step>` then `os.replace`'d to their canonical names). The manifest's step is the **only** authoritative source for resume; `energy.csv` is NEVER consulted to determine the saved step (its write cadence is ~10 ps via the reporter, while state saves are ~2 hr — using it would silently shorten resumed runs by hundreds of thousands of steps). A non-empty `state.xml` without a matching manifest fails fast and requires `force=True` to discard — the orphaned state cannot be paired with a freshly-built System.
